function r = rasterScan2(s, device, home_pos, x, y, res, freq, Amp, ave, totN, dt, no_frqs, no_harms)
    lvfig = 0;

    deviceAddress = 2;
    axisNumber = 0; % 0 = both, 1 = X-axis, 2 = Y-axis
    r = ones(length(x), length(y), no_frqs).*NaN;
    
    % Set the Lock-in Amp parameters and read off frequencies
    frq2 = example_connect_config_rob(device, freq, Amp, no_harms);
...
%             rTest = multiFreqCmplx(device, ave, no_frqs);
